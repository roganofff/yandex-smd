package quo.yandex.financialawareness.data.api
...
import quo.yandex.financialawareness.util.result.ErrorResponse
    ...
    suspend fun <T> call(
        ...
        return withContext(dispatcher) {
            ...
            repeat(MAX_RETRY_ATTEMPTS) { attempt ->
                try {
                    return@withContext ResultState.Success(apiRequest())
                } catch (throwable: Throwable) {
                    lastException = throwable

                    when (throwable) {
                        is IOException -> return@withContext ResultState.Failure(FailureReason.Network())
                        is HttpException -> {
                            if (throwable.code() in 500..599 && attempt < MAX_RETRY_ATTEMPTS - 1) {
                                delay(RETRY_DELAY_MS)
                            } else {
                                return@withContext ResultState.Failure(throwable.toFailureReason())
                            }
                        }
                        else -> return@withContext ResultState.Failure(FailureReason.Unknown(throwable.message))
                    }
                }
            }

            when (lastException) {
                is HttpException -> ResultState.Failure(lastException.toFailureReason())
                else -> ResultState.Failure(FailureReason.Unknown(lastException?.message))
            }
        }
    }
